...
reddit = asyncpraw.Reddit(client_id=config.REDDIT_CLIENT_ID,
                     client_secret=config.REDDIT_KEY,
...
async def buildEmbedMessage(_title, _headers, _contents, _color, _footer, _channel, _inline):
...
async def on_ready():
    ...
    print("Fetching anime legs...")
    try:
        subreddit = await reddit.subreddit("animelegs")
        async for submission in subreddit.hot(limit=100):
            submissionList.append(submission)
        print("Completed fetching anime legs on "+str(datetime.now().strftime("%H:%M:%S")))
    except:
        print("Something went wrong while trying to fetch anime legs"+".")
...
async def on_message(message):
    ...
    if not guild or channel.is_nsfw():
        ...
        if message.content.startswith("h!help"):
            ...
            "👉Gives a random image from r/animelegs",
        ...
        if message.content.startswith("h!title "):
            ...
            if len(resultArray) == 0:
                await buildEmbedMessage("Oewh.", ["Error: Something went wrong"], ["The search did not get any results or the request timed out."], discord.Colour.red(),"DEFAULT",message.channel, [False])
                return 0;
        ...
        if message.content.startswith("h!search ") or message.content.startswith("h!id "):
            ...
            if response == False:
                #Does not exist
                await buildEmbedMessage("Oewh.", ["Error: Something went wrong"], ["Your requested id could not be found or does not exist. Please try another id."], discord.Colour.red(),"DEFAULT",message.channel, [False])
        ...
        if message.content.startswith("h!credits"):
            title = "😍Hentei🌟credits😍"
            headers = ["🧠Idea and Code👩‍💻","🕹βeta Test🐞","💾Data and API backend🚀","🤑Want to support Hentei?💸","🏚Join the HenteiBot server!🗿"]
            contents = ["EpicGamer, AlphaAlpaka","Artfulrelic8, Sensei, LoneLarry, Aficon, Zegon","nhentai.net","Go and get 🤩HENTEI🌟PREMIUM🤩","https://discord.gg/uH7g5DfsF5"]
            color = discord.Color.gold()
            footer = "🍜Thanks for using Hentei!"
            inline = [False,False,False,False,False]
        ...
        if message.content.startswith("h!debug"):
            title = "Info on user:"
            headers = ["UserID","numbers?","Guild ID","Channel ID"]
        ...
        if message.content.startswith("h!legs"):
        ...
        if message.content.startswith("h!"):
            await buildEmbedMessage("Oewh.", ["Error: Something went wrong"], ["This is not a NSFW channel"], discord.Colour.red(),"DEFAULT",message.channel, [False])
